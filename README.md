...
ComfyUI 用 DARASK カスタムノード集。2つの実用ワークフローを軸にした8個のノード:
...
| 7 | DARASK Prompt Cell | DARASK/Prompt | プロンプト断片の1セル。複数チェイン/分岐で全組合せ展開 |
| 8 | DARASK Prompt Cell Output (CLIP Encode) | DARASK/Prompt | プロンプトチェインの終端。CONDITIONING を出力 |
...
### 7. DARASK Prompt Cell  *(カテゴリ `DARASK/Prompt`)*
...
### 8. DARASK Prompt Cell Output (CLIP Encode)  *(カテゴリ `DARASK/Prompt`)*
