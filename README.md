...
複数のLoRAを1ノードでスタック適用するノード。[rgthree の Power Lora Loader](https://github.com/rgthree/rgthree-comfy) と同等の機能を、ComfyUI 標準のウィジェット型(toggle / combo / number / button)だけで実装した安定版です。

#### 操作

* **`+ Add LoRA` ボタン** をクリックでLoRA行を追加。1行ごとに3つのウィジェットが追加されます:
  * `lora_N_on` — トグル(ON/OFF)
  * `lora_N` — コンボボックス(LoRAファイル選択、`folder_paths.get_filename_list("loras")` の一覧)
  * `lora_N_strength` — 数値ボックス(strength、範囲 -4 〜 4、刻み 0.05)
* **`− Remove Last` ボタン** で末尾の1行(3ウィジェット)をまとめて削除
* 各ウィジェットは ComfyUI 標準なので、ドラッグ / クリック / 直接入力すべて通常通り
...
* `lora_N_on = false` または `lora_N = "None"` または `lora_N_strength = 0` の行はスキップ
* LoRAファイル名は **fuzzy match** で解決:exact → basename(拡張子なし)→ substring の順
* 行は `lora_N` の N の昇順に適用

#### 既存 rgthree ワークフローとの互換性

Python 側は **2つの保存形式の両方を受け付けます**:

1. **Split format**(本ノードのデフォルト保存形式)— 各LoRA行が `lora_N`, `lora_N_on`, `lora_N_strength` の3キー
2. **Dict format**(rgthree Power Lora Loader と同じ)— `lora_N = {on, lora, strength, strengthTwo}` の1キー

rgthree で作成・保存されたワークフローを `DARASK Lora Loader` にコピーペーストで貼り直せばそのまま動きます。また `DARASK Exif Apply` 系はどちらの形式も認識するので、メタデータからLoRAリストを復元する経路も両対応。
...
| 機能 | デフォルト | 対応 ComfyUI 本体ノード |
|---|---|---|
| **Shift** (`ModelSamplingDiscreteFlow`) | ON, 5.0 (SD3/Anima/Wan) | `ModelSamplingSD3` / `ModelSamplingAuraFlow` / `ModelSamplingFlux` |
| **Temporal Score Rescaling** (TSR) — 動画特化のノイズ予測再スケーリング | OFF, k=1.0, σ=1.0 | `TemporalScoreRescaling` |
| **Epsilon Scaling** — exposure bias 補正 | OFF, factor=1.005 | `EpsilonScaling` (arXiv:2308.15321) |
| **CFG mode**(排他):`Rescale CFG` / `CFG Zero Star` | off | `RescaleCFG` / `CFGZeroStar` |

CFG mode はラジオ式(`sampler_cfg_function` は1つしか持てない仕様のため)、それ以外は post-CFG hook なので **スタック可能**。

#### 推奨値(Anima/Wan)

* `shift = 3.0 〜 8.0`(動きの一貫性が欲しいなら高め)
* `TSR k=1.0, σ=1.0` で十分(動きが破綻するなら k を上げる)
* `RescaleCFG = 0.7`(過飽和抑制)
* `CFG Zero Star` は CFG=1 付近で uncond を最適スケーリング、CFG=1動画用途で効く
