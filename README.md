...
* **mosaic 出力が出た時のチェックリスト**:
  * `prediction_weight = 0` にする(Taylor 外挿のみ、もっとも安定)
  * `warmup_steps >= polynomial_degree + 1` にする(degree=6 なら warmup を 7 以上)
  * `res_multistep` などの multistep サンプラーと組み合わせる場合は特に保守的に設定(キャッシュ予測が累積誤差を生む)
  * 効果検証のためにまず Step Cache をバイパス(ノードを mute)して Sampling Tuner のみで通常生成できることを確認
